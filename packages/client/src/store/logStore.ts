...
/**
 * Battle-log store. Keeps a flat list of human-readable entries for the
 * right-hand log panel. Separate from the game store so log updates don't
 * re-render the entire battle UI.
 */
...
interface LogStore {
  entries: LogEntry[];
  /** Add a manual entry (custom text). */
  addEntry: (entry: Omit<LogEntry, "id" | "timestamp">) => void;
  /** Convert a GameEvent into a log entry and append it. */
...
/**
 * Translate a GameEvent into a LogEntry. Returns null for events we don't
 * want to surface in the battle log (e.g. energy changes are noisy).
 */
function mapEventToEntry(
  ...
  switch (event.kind) {
    ...
    // Quiet events — not worth cluttering the log
    case "unit_healed":
    case "unit_buffed":
    case "unit_moved":
    case "card_discarded":
